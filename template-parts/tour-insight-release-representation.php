<?php
/**
 * Template part: Tour insight — Release representation (which releases are represented across the tour's setlists).
 * Used on tour archive only. Logic in includes/tour-functions.php (jww_get_tour_album_stats).
 *
 * @package JWW_Theme
 */
...
if ( ! function_exists( 'jww_get_tour_album_stats' ) ) {
	return;
}

$stats = jww_get_tour_album_stats( $tour_term->term_id );
...
<div class="tour-release-representation show-setlist-album-stats wp-block-group alignwide has-global-padding setlistAlbumStats" aria-labelledby="tour-release-representation-heading">
	<div class="show-setlist-album-stats-card">
		<div class="show-setlist-album-stats-header-wrapper">
			<h2 id="tour-release-representation-heading" class="wp-block-heading show-setlist-album-stats-heading"><?php esc_html_e( 'Release representation', 'jww-theme' ); ?></h2>
			<span>
				<span class="dashicons dashicons-chart-pie" title="<?php esc_attr_e( 'What releases are represented across all setlists we have for this tour. Counts are unique songs per release.', 'jww-theme' ); ?>"></span>
			</span>
		</div>

		<?php if ( $total_entries > 0 ) : ?>
			<div class="show-setlist-album-stats-chart" role="img" aria-label="<?php esc_attr_e( 'Release representation across this tour', 'jww-theme' ); ?>">
		...
		<?php foreach ( $groups as $group ) : ?>
			...
			$details_id = 'tour-album-stats-' . sanitize_title( $label ) . '-' . ( $album_id ? $album_id : 'group' );
			?>
